...
def pendingties(pending,weight=1,oxyz=(False,False,False,False),tol=1e-14,carbonbased=False):
    ...
    """
    if not oxyz[0]: #origin is not set
        ...
        if carbonbased:
            ...
            #everything
            tielist = range(len(pending))
        w = weight / len(tielist) #initial split, divide by number of atoms in a molecule.
        return w,tielist
...
def structuretoviews(pending,viewlength=None,weight=1.,done=None,oxyz=(False,False,False,False),tol=1e-14,carbonbased=False):
    ...
    if viewlength is None:
        # use all atoms
        viewlength = len(pending) # length of the molecule
...
def calculate_self_interaction(z):
    return 0.5 * z ** 2.4

def calculate_off_diagonal(z1, z2, pos1, pos2):
...
def update_list(molecule):
    """
    Generate a list of self and off-diagonal interactions for all atoms in a molecule.
    """
    interaction_list = []
    for i, (atom_type, position) in enumerate(molecule):
        zi = speciesdict[atom_type]
        interaction_list.append(calculate_self_interaction(zi))
        # Calculate interaction between new and previous atoms
        for j in range(i - 1, -1, -1):
            zj = speciesdict[molecule[j][0]]
            interaction_list.append(calculate_off_diagonal(zi, zj, position, molecule[j][1]))
